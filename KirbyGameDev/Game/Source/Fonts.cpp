...
int Fonts::Load(const char* texture_path, const char* characters, uint rows)
{
	...
	id = 0;
	for (; id < MAX_FONTS; ++id)
		if (fontsLoaded[id].texture == nullptr)
			break;
	...
	strcpy_s(fontsLoaded[id].table, MAX_FONT_CHARS, characters);
	font.totalLength = strlen(characters);
	font.columns = fontsLoaded[id].totalLength / rows;
...
{
	if (font_id >= 0 && font_id < MAX_FONTS && fontsLoaded[font_id].texture != nullptr)
	{
		app->tex->UnLoad(fontsLoaded[font_id].texture);
		fontsLoaded[font_id].texture = nullptr;
...
{
	if (text == nullptr || font_id < 0 || font_id >= MAX_FONTS || fontsLoaded[font_id].texture == nullptr)
	{
		LOG("Unable to render text with bmp font id %d", font_id);
		return;
	}

	const Font* font = &fontsLoaded[font_id];
	SDL_Rect spriteRect;
